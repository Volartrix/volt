...
// fuck idk what to call this. Well it return something like this = "CPL=0 II=0 A20=1"
...
char* segment_str(uint64_t cs, uint64_t ss, uint64_t ds) {
    ...
    // Build the result string with formatted output for CS, SS, and DS segment descriptors
...
char* get_control_registers() {
    ...
    // Format the result string
    snprintf(result, sizeof(result),
             "CR0 = 0x%.8lX CR2 = 0x%.8lX CR3 = 0x%.8lX CR4 = 0x%.8lX",
             cr0, cr2, cr3, cr4);
