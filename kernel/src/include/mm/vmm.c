...
void map_sections(uint64_t pml4[]) {
    ...
    for (size_t entry = 0; entry < num_memmap_entries; entry++) {
        ...
        if (entry_type == LIMINE_MEMMAP_USABLE ||
            ...
            map_pages(pml4, memmap_entries[entry].base + hhdm_offset, memmap_entries[entry].base, memmap_entries[entry].length / 4096, KERNEL_PFLAG_PRESENT | KERNEL_PFLAG_WRITE);
        }
    }
}
